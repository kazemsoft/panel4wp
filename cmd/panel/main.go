...
<section><h2>Sites</h2>{{if not .Sites}}<p class="muted">No sites yet.</p>{{else}}<table><thead><tr><th>Site</th><th>Status</th><th>Actions</th></tr></thead><tbody>{{range .Sites}}<tr><td><strong>{{.Title}}</strong><br><a href="{{if hasSuffix .Domain ".localhost"}}http{{else}}https{{end}}://{{.Domain}}" target="_blank" rel="noopener">{{.Domain}}</a><br><small>{{.ID}}</small>{{if .Error}}<p class="error">{{.Error}}</p>{{end}}</td><td><span class="badge">{{.Status}}</span></td><td>
...
func (a *app) callWorker(path string, body any) error {
...
func (a *app) createSite(w http.ResponseWriter, r *http.Request) {
	...
	site := core.Site{ID: id, Domain: domain, Title: strings.TrimSpace(r.FormValue("title")), AdminEmail: strings.TrimSpace(r.FormValue("email")), Status: core.StatusCreating, CreatedAt: time.Now().UTC()}
	...
	if err := a.callWorker("/create", core.CreateRequest{Site: site, DBPassword: dbPassword, AdminPassword: adminPassword}); err != nil {
...
func (a *app) siteAction(w http.ResponseWriter, r *http.Request) {
	...
	if action == "retry" {
		...
		_ = a.store.Put(site)
		err = a.callWorker("/create", core.CreateRequest{Site: site, DBPassword: dbPassword, AdminPassword: adminPassword})
		if err == nil {
			site.Status = core.StatusRunning
			_ = a.store.Put(site)
		...
		err = a.callWorker("/action", map[string]string{"id": id, "action": action})
		if err == nil {
			if action == "delete" {
				_ = a.store.Delete(id)
			} else if action == "stop" {
